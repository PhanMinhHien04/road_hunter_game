...
#define AK_LOGO_TEXT		(AK_LOGO_AXIS_X + 4)
	...
	view_render.setCursor(AK_LOGO_AXIS_X, 8);
	view_render.print("   __    _  _ ");
	view_render.setCursor(AK_LOGO_AXIS_X, 16);
	view_render.print("  /__\\  ( )/ )");
	view_render.setCursor(AK_LOGO_AXIS_X, 24);
	view_render.print(" /(__)\\ (   (");
	view_render.setCursor(AK_LOGO_AXIS_X, 32);
	view_render.print("(__)(__)(_)\\_)");
	view_render.setCursor(AK_LOGO_TEXT, 48);
...
void scr_startup_handle(ak_msg_t *msg) {
	...
	case AC_DISPLAY_INITIAL: {
		...
		view_render_screen(&scr_startup);	/* force-render logo now; don't rely on scr_mng_render_screen()'s
											 * screen_render_started flag, which may already be false on some
											 * reset paths and would silently skip the very first frame. */
	...
	case AC_DISPLAY_BUTON_MODE_PRESSED: {
		...
		SCREEN_TRAN(scr_rh_title_handle, &scr_rh_title);
	} break;
