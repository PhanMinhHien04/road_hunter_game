...
void scr_mng_ctor(scr_mng_t* scr_mng, screen_f init_scr, view_screen_t* scr_obj) {
	...
	if (screen_manager->screen != scr_idle_handle) {
...
void scr_mng_dispatch(ak_msg_t* msg) {
	...
	if (msg->sig == AC_DISPLAY_SCREEN_IDLE && screen_manager->screen != scr_idle_handle) {
		timer_remove_attr(AC_TASK_DISPLAY_ID, AC_DISPLAY_SCREEN_IDLE);
...
void scr_mng_back_no_entry() {
	screen_f target;
	view_screen_t* scr_obj;
	ak_msg_t screen_msg_restore;
	...
	/* restore previous screen without triggering its SCREEN_ENTRY handler */
	view_screen = scr_obj;
	screen_manager->screen = target;

	screen_msg_restore.sig = SCREEN_RESTORE;
	screen_manager->screen(&screen_msg_restore);
